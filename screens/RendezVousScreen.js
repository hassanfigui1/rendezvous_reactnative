...
export default function RendezVousScreen() {
  ...
  useEffect(() => {
    fetch(`${API_BASE_URL}/api/centres?page=0&size=20`, {
      headers: {
  ...
  const handleConfirm = (date) => {
    ...
    if (selectedCentre) {
      ...
      console.error("No centre selected.");
    }
  };

  const fetchCreneausForCentre = (centreId, formattedDate) => {
    fetch(`${API_BASE_URL}/api/centres/${centreId}/creneaus`, {
      headers: {
        Authorization: `Bearer ${userToken}`,
      },
    })
  ...
  const handleSubmit = () => {
    ...
    if (!selectedDate || !selectedCentre || !selectedCreneau || !user) {
      console.error("Please select a date, centre, creneau, and user.");
      return;
    }
    ...
    fetch(`${API_BASE_URL}/api/rendezvous`, {
      method: "POST",
      headers: {
